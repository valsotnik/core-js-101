...
function isInsideCircle(circle, point) {
  // throw new Error('Not implemented');
  let dist = (circle.center.x - point.x) ** 2;
  dist += (circle.center.y - point.y) ** 2;
...
function findFirstSingleChar(str) {
  ...
  return array.filter((elem) => array.indexOf(elem) === array.lastIndexOf(elem))[0];
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < strCCN.length; i += 1) {
    let cardNumber = parseInt(strCCN[i], 10);

    if (!((strCCN.length - i) % 2)) {
      cardNumber *= 2;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < brackets.length;) {
    if (curr.indexOf(brackets[i]) !== -1) {
      curr = curr.replace(brackets[i], '');
      i = 0;
    } else i += 1;
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const res = [];
  arrPathes[0].forEach((el, i) => {
    if (arrPathes.every((path) => path[i] === el)) {
      res.push(`${el}/`);
    }
  });
  return res.join('');
